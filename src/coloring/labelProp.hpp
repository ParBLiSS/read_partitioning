  ...
  namespace coloring
  {
    ...
    class ccl 
    {
      public:
        ...
        using tupleType = std::tuple<pIdtype, pIdtype, nodeIdType>;
      ...
      public:
        ...
        ccl(std::vector<ET> &edgeList, const mxx::comm &c) : comm(c.copy()) 
        {
          //nodeIdType and TODO should match
          //If they don't, modify the class type or the edgeList type
          //static_assert(std::is_same<TODO, nodeIdType>::value, "types must match");
        ...
        void compute(std::vector< tupleType>& rValue) {
          compute();
          rValue.swap(tupleVector);
        }
      ...
      private:
         ...
         */
        template <typename ET>
        void convertEdgeListforCCL(std::vector<ET> &edgeList)
        {
          ...
          for(auto it = edgeList.begin(); it != edgeList.end(); it++)
            tupleVector.emplace_back((*it).vertexU, MAX_PID, (*it).vertexV);;
